    ...
    <div className="space-y-6 px-4 py-6 md:px-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-muted-foreground">Jami yuborilgan</p>
            <p className="text-2xl font-bold">{messages.length}</p>
          </div>
          <MessageSquare className="w-8 h-8 text-primary opacity-70" />
        </Card>
        <Card className="p-4 flex items-center justify-between">
          <div>
      ...
      <Dialog
        open={sendOpen}
        onOpenChange={(o) => {
          if (!o) { setSendOpen(false); setText(""); setSelectedIds([]); setSelectedGroup(""); setSearchStudent(""); }
        }}
      >
